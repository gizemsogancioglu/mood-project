...
def read_mood_preds():
    ...
    for t in mood_model.class_dimensions:
        for split in ['training', 'validation', 'test']:
            mood_preds[t].append(pd.read_csv("../predictions/" + t + "_preds_" + split + ".csv").rename
                                 (columns={'0': 'low_'+t, '1': 'medium_'+t, '2': 'high_'+t}))
    return mood_preds
...
def run():
    ...
    for i in [0, 1, 2]:
        features.append(
            pd.concat([mood_preds['valence'][i], mood_preds['arousal'][i], mood_preds['likeability'][i]], axis=1)
                .drop(columns=['Unnamed: 0']))
    return ebm_regression(features)



if __name__ == "__main__":
    if not os.path.isfile("../predictions/valence_preds_training.csv"):
        mood_model.run()
    else:
        print("Mood+likeability predictions are already in the following folder :"
              " (predictions/valence_predictions.csv)."
              " Skipping training of first-level mood classifier.. ")
    run()
